...
class BaselineTest4(BaseModule):
    ...
    def __init__(self, 
        ...
        self.bb = nn.TransformerEncoder(block, 2, enable_nested_tensor=False)

        block = nn.TransformerEncoderLayer(dim, n_heads, device=device, dim_feedforward=dim_ff,
                                                batch_first=True, dropout=t_dropout)
        self.op = nn.TransformerEncoder(block, 2, enable_nested_tensor=False)
        ...
        self.pooling_size = 4
        self.pooling_embed = nn.Embedding(self.pooling_size, dim)
   
        self.attn = nn.MultiheadAttention(dim, 8, 0.1, batch_first=True)
        self.key_lin = nn.Linear(dim, dim)
        self.val_lin = nn.Linear(dim, dim)

        self.merger = nn.Sequential(
            nn.Dropout(),
            nn.Linear(self.pooling_size * dim, dim)
        )
    ...
    def forward(self, x):
        ...
        output = self.bb(output, src_key_padding_mask=mask)
        ...
        # 4 D
        query = self.pooling_embed(torch.arange(self.pooling_size, device=output.device, dtype=torch.int)).unsqueeze(0)
        query = self.pos_embed(query)

        # B*I 4 D
        query = query.expand(batch_size * inst_size, -1, -1)

        output = output.masked_fill(op_seq_mask.unsqueeze(-1).unsqueeze(-1), 1)
        mod_mask = mask.masked_fill(op_seq_mask.unsqueeze(-1), False)

        #  B*I 4 D
        output = self.attn(query, self.key_lin(output), self.val_lin(output), mod_mask, False)[0]
        output = output.masked_fill(op_seq_mask.unsqueeze(-1).unsqueeze(-1), 0)

        #  B 4*I D
        output = output.view(batch_size, 4 * inst_size, -1)
        output = self.pos_embed(output)
        op_seq_mask = op_seq_mask.view(batch_size, inst_size)
        mod_mask = op_seq_mask.repeat(1, 4)
        output = self.op(output, src_key_padding_mask=mod_mask)
