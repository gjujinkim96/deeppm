...
class BaselineTest5(BaseModule):
    ...
    def __init__(self, 
        ...
        self.single = nn.TransformerEncoder(block, 2, enable_nested_tensor=False)

        block = nn.TransformerEncoderLayer(dim, n_heads, device=device, dim_feedforward=dim_ff,
                                                batch_first=True, dropout=t_dropout)
        self.mixed = nn.TransformerEncoder(block, 2, enable_nested_tensor=False)

        block = nn.TransformerEncoderLayer(dim, n_heads, device=device, dim_feedforward=dim_ff,
                                                batch_first=True, dropout=t_dropout)
        self.third_single = nn.TransformerEncoder(block, 1, enable_nested_tensor=False)

        block = nn.TransformerEncoderLayer(dim, n_heads, device=device, dim_feedforward=dim_ff,
                                                batch_first=True, dropout=t_dropout)
        self.third_mixed = nn.TransformerEncoder(block, 1, enable_nested_tensor=False)
    ...
    def forward(self, x):
        ...
        mixed_output = self.mixed(output, src_key_padding_mask=mask)
        mixed_output = mixed_output.masked_fill(mask.unsqueeze(-1), 0)
        mixed_output = mixed_output.view(batch_size, inst_size, seq_size, -1)
        ...
        output = output.masked_fill(op_seq_mask.unsqueeze(-1).unsqueeze(-1), 1)
        mod_mask = mask.masked_fill(op_seq_mask.unsqueeze(-1), False)
        t_out = self.third_single(output, src_key_padding_mask=mod_mask)
        t_out = t_out.masked_fill(op_seq_mask.unsqueeze(-1).unsqueeze(-1), 0)
        t_out = t_out.view(batch_size, inst_size, seq_size, -1)
        mask = mask.view(batch_size, inst_size, seq_size)
        t_out = t_out.masked_fill(mask.unsqueeze(-1), 0)

        t_out = t_out.view(batch_size, inst_size * seq_size, -1)
        mask = mask.view(batch_size, inst_size * seq_size)
        t_out = self.third_mixed(t_out, src_key_padding_mask=mask)
        t_out = t_out.masked_fill(mask.unsqueeze(-1), 0)
        t_out = t_out.view(batch_size, inst_size, seq_size, -1)
