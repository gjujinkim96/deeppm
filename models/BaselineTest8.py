...
class BaselineTest8(BaseModule):
    ...
    def __init__(self, 
        ...
        self.single = nn.TransformerEncoder(block, 4, enable_nested_tensor=False)

        block = nn.TransformerEncoderLayer(dim, n_heads, device=device, dim_feedforward=dim_ff,
                                                batch_first=True, dropout=t_dropout)
        self.mixed = nn.TransformerEncoder(block, 4, enable_nested_tensor=False)

        block = nn.TransformerEncoderLayer(dim, n_heads, device=device, dim_feedforward=dim_ff,
                                                batch_first=True, dropout=t_dropout)
        self.merger_tr = nn.TransformerEncoder(block, 1, enable_nested_tensor=False)
    ...
    def forward(self, x):
        ...
        # B I 2 D
        output = torch.stack((single_out, mixed_output), dim=2)

        # B I*2 D
        output = output.view(batch_size, 2*inst_size, -1)
        output = self.pos_embed(output)

        output = self.merger_tr(output, src_key_padding_mask=op_seq_mask.repeat(1, 2))
        output = self.prediction(output).squeeze(-1)
        output = output.masked_fill(op_seq_mask.repeat(1, 2), 0)
