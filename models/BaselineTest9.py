...
class BaselineTest9(BaseModule):
    ...
    def __init__(self, 
        ...
        block = nn.TransformerEncoderLayer(dim, n_heads, device=device, dim_feedforward=dim_ff,
                                                batch_first=True, dropout=t_dropout)
        self.final = nn.TransformerEncoder(block, 2, enable_nested_tensor=False)
    ...
    def forward(self, x):
        ...
        output = single_out + mixed_output
        output = output.view(batch_size, inst_size * seq_size, -1)
        mask = mask.view(batch_size, inst_size * seq_size)
        output = self.final(output, src_key_padding_mask=mask)
